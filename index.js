...
const CONTENT = [
  ...
  '- 启动：TcpStream 探测 127.0.0.1:3080（DSH_DESKTOP_PORT 可覆盖）——已监听复用、空闲 spawn；',
  ...
  '仓库 desktop/scripts/acceptance.sh / .ps1 自动跑三条路径：A 复用（3080 已有服务，退出不杀）、',
  ...
  '## 七、桌面端与会话的关系',
  '',
  '桌面壳是同一个 dsh 服务的壳，不是新会话：复用 3080 后与浏览器同后端、同会话存储',
  '（~/.dsh/sessions），历史对话/上下文完整保留；冷启动也恢复。避免两边同时操作同一会话。',
...
export function apply(ctx) {
  ...
  ctx.effect(() =>
    skills.register({
      ...
      title: 'DSH 桌面壳「小南梁」',
      description:
        '把 DeepSeek Harness Web GUI 封装成 Tauri 2 桌面应用（macOS + Windows 双平台），含托盘常驻、单实例、子进程生命周期、任务完成通知；内置 DSH 安装分平台、国内镜像加速（rustup/cargo/npm/GitHub/NSIS）、subagent 哨兵下载判定、Windows 无管理员工具链方案。当用户想把 DSH 做成桌面应用、搭 Tauri 项目、或在 Windows 新环境配置 Rust/Node 工具链时使用。',
      whenToUse:
        '用户想把 DeepSeek Harness 或任意本地 Web 应用封装成桌面应用；需要在 Windows 新环境配置 Rust/Node 工具链（含国内镜像加速）；需要无管理员权限构建 Tauri 项目；遇到 dmg/MSI 打包、SmartScreen、单实例锁等桌面壳坑时。',
